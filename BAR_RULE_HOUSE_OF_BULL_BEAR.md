...
**Status:** Under active case-study cross-verification against `bar_rule_sim2.py`. REAR/REAR RE-ENTER have been removed from this logic entirely (see §9) — only TZ GREEN/TZ RED cycles exist.
...
TZ GREEN → TZ GREEN 2 → RED1 → RED2 → [ BAR → BAR 2 ] ↻ (repeating engine)
                                            ↓
                              BAR SL → BAR SL2 → fresh TZ GREEN (see §9)
```
...
- RED1 SL: High clears `red1.ref_high` by ≥ 0.20, Close holds above → `red1.active = False`.
- **SL always beats RED1/RED2** — checked first every candle.
- RED1 only ever attaches once the current generation's own "...2" object exists: TZ GREEN 2 the first time; BAR 2 on every subsequent generation. **RED1/RED2 formation requires an ACTIVE BAR** — once a BAR SL has fired and no fresh BAR has yet reformed, RED1/RED2 formation is blocked entirely; it does not fall back to using the anchor (TZ GREEN) as a substitute once at least one BAR has ever existed.
...
- BAR SL HH/LL are tracked (ordinary tracking, both sides, simple threshold — no "…2" object exists at the SL level to govern/silence either side).
- **There is no "BAR 2 SL2" concept** (BAR 2 itself has no further failure state beyond the base BAR SL/SL2 below).
...
2. **RED2 forecloses that path permanently**, whenever it fires (before, on, or after the BAR 2 SL): "NEW BAR 2" becomes unavailable for that generation. The only way forward is a full next generation — BAR(N+1) → BAR 2(N+1). The earlier BAR(n)/BAR 2(n) goes **dormant** (not terminated) until BAR 2(N+1) validly confirms.
...
- **(B) BAR SL(n) recovery path** — a fresh qualifying breakout off BAR SL(n)'s own reference forms BAR(N+1) as a separate lineage, independent of RED1/RED2, and independent of whether BAR(n)'s own reactivation (a fresh BAR forming under the same label off its own SL, per §9) has fired yet (can happen before or after it).
...
- If BAR(n) already has a BAR SL: it stays active/racing (own SL2 still possible, BAR SL HH/LL still tracked) until the **earlier of** BAR(n)'s own reactivation firing or BAR 2(N+1) validly confirming.
...
- Case 2 (BAR SL(n) exists, own reactivation still pending): `BAR(n) - BAR2(n) - BARSL(n) - BAR(n+1) - BAR2(n+1) - RED1 + BARSL2(n)` — BAR(n) remains active/not terminated the whole time; its own SL2 can still fire independently of what's happening at n+1.
- Case 3A: `... - BAR(n+1) - BAR2(n+1) + [BAR(n)'s own reactivation]` → BAR(n) terminates when its own reactivation fires on/after BAR2(n+1) confirming.
- Case 3B: `... - BAR(n+1) + [BAR(n)'s own reactivation] - BAR2(n+1)` → BAR(n) terminates when its own reactivation fires before BAR2(n+1) confirms.
- Rule: **BAR(n) terminates at the earlier of its own reactivation firing, or BAR 2(N+1) validly confirming.** If BAR(n) never had an SL, only the BAR 2(n+1)-confirms trigger applies.
...
- BAR SL2 sub-object: on BAR SL formation, a fresh SL-tracking object starts, initialized from the *SL-triggering candle's own* High/Low (not the original BAR's reference).
- Each subsequent day, in priority order:
  1. **Reactivation** (a fresh BAR forming via the plain SL-recovery breakout, §8 trigger B) is checked first — if it fires, the SL object is discarded and a fresh BAR(N+1) takes over; SL2 is not checked that day.
  ...
  3. Otherwise, ordinary **BAR SL HH / BAR SL LL** tracking on the SL object (both sides, simple threshold).
...
There is really **one generational engine**: `BAR → BAR 2 → RED1 → RED2 → next BAR`, repeating for as long as each BAR SL2 recovers via the direct reactivation path (§8 trigger B). TZ GREEN/TZ GREEN 2 is the very start of a cycle. **BAR SL2 (permanent terminal failure, §9) resets the whole thing back to a fresh TZ GREEN search** — there is no recovery label that continues the old cycle; the engine starts over from scratch. (§1–§10 describe this engine as it runs under House of Bull, where the generational label is `BAR`/`BAR 2`; §11 defines House of Bear's mirror, where the same engine's label is `SAR`/`SAR 2` instead — see §11's mapping note.)

---

## 11. House of Bull vs. House of Bear

Two full mirror-image structures, mostly sharing the same event vocabulary (the one deliberate exception: House of Bull's generational label is `BAR`/`BAR 2`, House of Bear's is `SAR`/`SAR 2` — see the mapping below). Both houses' engines always run and always log (§12) — "active" means whichever house's own chain currently reaches furthest, not that the other stops being tracked.
...
- Every downstream mechanic built for Bull in §4–§9 under the name `BAR`/`BAR 2` (BAR SL/no SL2-at-the-BAR2-level, NEW-BAR-2-vs-RED2-forecloses-it, BAR(N+1) triggers and BAR(n) termination rules, BAR SL2 resetting to a fresh anchor) applies **exactly, direction-flipped, to `SAR`/`SAR 2`** in House of Bear — read every `BAR`/`BAR 2`/`BAR SL`/`BAR SL2`/`NEW BAR 2` in §4–§9 as `SAR`/`SAR 2`/`SAR SL`/`SAR SL2`/`NEW SAR 2` when applying those rules under House of Bear. **SAR SL2 resets unconditionally to a fresh `TZ RED` search**, mirroring BAR SL2 → fresh `TZ GREEN`.
...
**Which house is active initially:** determined purely by whichever chain (TZ GREEN's or TZ RED's own independent anchor search) completes its own sequence through to a `BAR` first. Both can — and typically do — produce events on the very same early dates before this is settled (see the case-study table in §14).
...
**After a switch — reclaiming active status is different from mere tracking.** The house that just lost active status does **not** stop being tracked or logged (above, this section, already establishes both houses always run). But to become the *active* house again — not just to keep producing shadow-tracked anchor events — that house must complete a **brand-new anchor from scratch** (a fresh `TZ GREEN` for Bull, a fresh `TZ RED` for Bear), not a continuation of its old pre-switch lineage. This is now the *only* way any cycle restarts, whether from a house-switch or from a BAR/SAR SL2 (§9) — there is no other recovery path.
...
- This is a continuous, permanent feature of the output, not just a start-of-dataset phenomenon — both houses' events keep appearing side-by-side for the life of the dataset, per §12's repeating rebirth/termination cycle, not only before the first switch ever happens.

## 14. Worked case-study table (from the user, confirmed correct)
...
15/01     BULL + BEAR  BAR 2 + GREEN 2
              (GREEN 2 may not appear this same day, or may appear a day or two later,
               if BAR 2 closes above TZ RED's HH reference — see §12's rebirth/termination cycle)
```
