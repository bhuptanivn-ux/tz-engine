...
- **(a) Fresh TZ GREEN(N+1) cycle**: a wholly new anchor search, unrelated to the dead generation's own reference — the ordinary base-case breakout (Low ≥ PrevLow, High > PrevHigh + 0.20, Close ≥ PrevHigh), exactly like day 1. This is the *only* available path if the dead generation was a plain `BAR`/`SAR` (not already a ladder rung) that never reached its own "2" stage — the very first promotion onto the ladder still needs `BAR 2`/`SAR 2`'s own reference to exist (confirmed against 15-02-2021: a plain `BAR` dies pre-"2", no REAR possible, only a fresh `TZ GREEN`). **Older-lineage precedence**: if this fresh anchor's own breakout would fire on the exact same day an EXISTING, older lineage's pending path-(b) recovery also resolves, the fresh anchor does NOT form that day at all — only the older lineage's recovery is recorded. The fresh-anchor search keeps waiting for a later day that doesn't collide (confirmed against 10-03-2022/11-03-2022, see §14).
...
- **Older-lineage precedence, generalized: a fresh TZ GREEN(N+1)/TZ RED(N+1) anchor does not form at all on a day its breakout coincides with an EXISTING, older lineage's own pending REAR BUY/SELL (or RE ENTER) recovery resolving — confirmed by the user, correcting an intermediate version of this fix.** I had first implemented "older wins" as a pullback-attach-only tiebreak (letting both fronts form, deferring only a later shared RED1/GREEN1). The user rejected this against 10-03-2022 ("Since both have occurred on the same day, why to consider TZ GREEN? No need."): the fresh anchor must not be recorded/formed at all that day if it collides with an older lineage's recovery — not merely have its pullback-eligibility deferred. `awaiting_fresh_anchor` simply keeps waiting (stays `True`) until a day that does *not* collide with any existing lineage's same-day `rear_recovery` resolution — confirmed: `TZ GREEN` was suppressed 10-03-2022 (coinciding with `REAR BUY`'s recovery that day) and successfully formed the very next day, 11-03-2022, once `REAR BUY`'s `rear_recovery` had already resolved (nothing left to collide with). Implemented via a `rear_recovery_would_resolve` check across all lineages, gating the top-level fresh-anchor-search block. Same-day collisions between two fronts that are BOTH already-existing lineages (not a fresh anchor) are unaffected — that's the ordinary permanent dual-track (§8), not this precedence rule, which is specific to a *fresh* anchor attempt losing to an *existing* lineage's same-day recovery.
