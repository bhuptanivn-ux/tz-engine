...
- **GOVERNED single-sided inner tracking — the opposite of BAR 2, confirmed by the user.** Unlike BAR 2/SAR 2 (ungoverned, both sides forever — see §5), TZ GREEN 2/TZ RED 2's own INNER reference only ratchets on its FAVORABLE side: `TZ GREEN 2 HH` for the bullish anchor, `TZ RED 2 LL` for the bearish one. The adverse-inner side is silenced entirely (confirmed against 15-02-2022: `TZ RED 2 HH` must NOT print). This keeps running for as long as the lineage's gen hasn't taken over (see retirement, below), regardless of whether a RED1/GREEN1 has already attached — a pullback attaching does not pause or retire the anchor's own tracking.
- **The OUTER reference keeps ratcheting continuously — also the opposite of BAR/SAR's frozen outer.** Once TZ GREEN 2/TZ RED 2 forms, the anchor's OUTER (the pre-2 anchor's own reference, frozen at the moment of "2" formation) keeps extending on the ADVERSE side for as long as the anchor is tracked at all: `TZ GREEN LL` (bearish-direction adverse move against a bullish anchor) / `TZ RED HH` (bullish-direction adverse move against a bearish anchor), same `ANY` = 0.01 threshold, no Low/Close requirement. This directly feeds the deep-SL threshold (`min`/`max` of outer and inner) so it always reflects the true historical extreme rather than staying pinned to the original formation-day value — confirmed against 14-02-2022 (`TZ RED HH` needed so the deep threshold correctly reflects that day's new high) — and it is also the reference the support/resistance highlighting reads (§15): `TZ GREEN LL`/`TZ RED HH` are exactly the "base-level" events that rule already named.
- **Two-tier detection, but only the deep tier has a consequence** — confirmed by the user's explicit correction (previously this spec had the shallow tier behave exactly like BAR 2's, with its own recovery-awaiting window; that was wrong):
  - `TZ GREEN 2 SL` (shallow): Low breaks TZ GREEN 2's own inner threshold by ≥ 0.20, Close doesn't reclaim, *and* the deep threshold isn't also breached the same day. **This does not stop or restart anything.** It is logged, but the anchor Struct is left exactly as it was — no recovery window, no reforming "TZ GREEN 2", no effect on `alive`, no change to its reference values (so the SAME shallow breach can log again on a later day, which is expected — it carries no state). RED1/RED2/BAR/BAR 2/REAR BUY/REAR BUY 2/etc. (or their House-of-Bear mirrors) never depend on the anchor's own High/Low/reference for *their own* trigger conditions (§3) — but that is a separate fact from whether the anchor keeps tracking for its own sake, which it does (see the deep tier, next).
...
**TZ GREEN 2's shallow SL is a non-event, but its DEEP SL keeps mattering even mid-pullback (01/2022 case study, confirmed exact numbers, corrected per the user's explicit rule statement):**
```
DATE      HOUSE OF     EVENT
...
08/01     BULL + BEAR  TZ GREEN SL + TZ RED 2 LL + GREEN1 SL  [GREEN1 (attached 06/01) hits its OWN
                                                 SL on its own schedule -- nothing to do with TZ RED
                                                 2's state, which is separately still ticking its
                                                 ordinary LL ratchet (unaffected by the earlier
                                                 no-op shallow SL). Earlier drafts of this spec had
                                                 this print "TZ GREEN SL + TZ RED 2" (a fresh TZ RED 2
                                                 "recovering") -- confirmed wrong: TZ RED 2 SL has no
                                                 recovery of its own to await]
09/01     BEAR         TZ RED 2 LL              [ordinary ratchet continues -- GREEN1 is dead, no
                                                 fresh attach qualifies this day]
10/01     BULL + BEAR  TZ GREEN + TZ RED 2 SL + GREEN1  [a FRESH GREEN1 attach -- not GREEN2
                                                 continuing off the dead 06/01 GREEN1; a dead pullback
                                                 cannot resolve into GREEN2, only a live one can. TZ
                                                 RED 2's shallow SL logs again too -- no state, so the
                                                 same non-event can recur]
...
15/01     BULL + BEAR  RED1 + TZ RED            [Bull's RED1 attaches against TZ GREEN 2 (unrelated to
                                                 ...
                                                 its anchor's own deep SL ended it. An earlier draft of
                                                 this spec wrongly had the DEAD lineage's pullback
                                                 keep resolving through GREEN2 into a "SAR" here --
...
- **The anchor's INNER post-"2" tracking is GOVERNED (favorable side only), and its OUTER reference now ratchets continuously — both confirmed by the user, correcting the §2 mirror-of-BAR-2 assumption.** Previously TZ GREEN 2/TZ RED 2 was assumed to track ungoverned dual HH/LL exactly like BAR 2/SAR 2 (§5) — wrong for the INNER reference: only the favorable side (`TZ GREEN 2 HH` bullish / `TZ RED 2 LL` bearish) ratchets; the adverse-inner side is silenced (confirmed against 15-02-2022: `TZ RED 2 HH` must not print). Separately, the OUTER reference — frozen forever for the GEN once set (§6, reconfirmed against 25-01 through 30-01-2022) — instead keeps ratcheting continuously on the adverse side for the ANCHOR (`TZ GREEN LL` / `TZ RED HH`), confirmed against 14-02-2022 (`TZ RED HH` needed so the deep-SL threshold reflects that day's true extreme). This anchor outer ratchet is also the reference the support/resistance highlighting reads (§15).
