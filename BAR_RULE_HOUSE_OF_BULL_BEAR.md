...
- **(a) Fresh TZ GREEN(N+1) cycle**: a wholly new anchor search, unrelated to the dead generation's own reference — the ordinary base-case breakout (Low ≥ PrevLow, High > PrevHigh + 0.20, Close ≥ PrevHigh), exactly like day 1. This is the *only* available path if the dead generation was a plain `BAR`/`SAR` (not already a ladder rung) that never reached its own "2" stage — the very first promotion onto the ladder still needs `BAR 2`/`SAR 2`'s own reference to exist (confirmed against 15-02-2021: a plain `BAR` dies pre-"2", no REAR possible, only a fresh `TZ GREEN`).
...
- **Pullback-attach precedence when two lineages' fronts are born the identical day**: confirmed by the user — if a fresh RED1/GREEN1 becomes eligible against two DISTINCT lineages' fronts on a day where both fronts happen to have been created (formed) on the exact same earlier day, only the OLDER lineage (by which one's `Lineage` object was created first) actually attaches it; the newer lineage's side stays silent that day. This does not apply when the two fronts were formed on different days (even if both lineages are simultaneously eligible) — that case still gets the dual-tag treatment (§8's "Output disambiguation," unaffected). Implemented via `Struct.formed_day`/`Lineage.created_day` and a precedence-resolution pass after each day's per-lineage loop. Not yet exercised against a real date (10-03-2022's `TZ GREEN` and `REAR BUY` are both front-forming events, not a shared pullback attach) — needs confirming the next time it actually fires.
