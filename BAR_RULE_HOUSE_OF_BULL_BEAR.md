...
**Exception — RED2 already fired before the shallow SL**: this lightweight "NEW BAR 2 reforms directly" path is only available if RED1→RED2 has **not yet** confirmed for this generation. If RED2 *already* fired (setting `gen_pending`) before `BAR 2 SL` hits, the lighter recovery is foreclosed entirely — a **full fresh BAR** starts instead, directly off the already-set `gen_pending`, the moment the ordinary breakout condition next confirms (no `bar2_recovery` awaiting-state is created at all). Confirmed against 08–13-02-2022: RED1 attaches to BAR 2 on 08-02, RED2 confirms on 10-02, `BAR 2 SL` (shallow) fires on 11-02 — and because RED2 already fired, 12-02 starts a full new `BAR` cycle rather than awaiting a reformed `BAR 2`.
