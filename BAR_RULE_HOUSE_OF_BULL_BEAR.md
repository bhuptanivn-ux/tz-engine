...
- **(b) The escalation ladder**: available whenever the dead generation *had* reached its own "2" stage, **or the dead generation was already a rung of the ladder itself** (`REAR BUY`/`SELL` or a `RE ENTER` rung), regardless of whether that particular rung reached its own "2" — confirmed by the user against 06-03-2022/10-03-2022: `REAR BUY` (formed 02-03-2022, own High 315 on 03-03) died via a further deep SL on 06-03 *before* ever reaching `REAR BUY 2`, and still escalated to `REAR BUY RE ENTER`, reforming on 10-03-2022 directly above that plain (pre-"2") 315 reference (High 318 > 315.20, Low 313.5 ≥ PrevLow, Close 317.5 ≥ 315) — **"only REAR BUY SL was enough for REAR RE ENTER BUY to occur," no `REAR BUY 2` needed.** So the "needs its own '2'" requirement applies *only* to the first promotion (plain `BAR`/`SAR` → `REAR BUY`/`SELL`); every escalation after that (rung → next rung) uses whatever reference the dying rung had tracked, "2"-stage or not. The recovery reforms directly above that reference (same breakout shape: Low ≥ PrevLow, High > the reference + 0.20, Close ≥ that reference), and the recovery's *name* escalates one rung up a capped ladder depending on what level just failed. **The name is house-specific — House of Bull uses `REAR BUY`, House of Bear uses `REAR SELL`** (confirmed by the user; earlier drafts of this spec used a single generic `REAR` name for both houses, which was wrong):
...
- **Only the FIRST promotion onto the REAR ladder needs the dying gen's own "2" — every escalation after that does not, confirmed by the user against 06-03-2022/10-03-2022.** `REAR BUY` (formed 02-03-2022) died via a further deep SL on 06-03 before ever reaching `REAR BUY 2`; it still escalated to `REAR BUY RE ENTER`, which reformed on 10-03-2022 directly above `REAR BUY`'s own plain (pre-"2") High of 315 (from 03-03) — "only REAR BUY SL was enough for REAR RE ENTER BUY to occur." This does not reopen the already-confirmed 15-02-2021 case (a plain `BAR` — not yet on the ladder — dying pre-"2" still has no REAR path, only a fresh `TZ GREEN`): the distinction is the FIRST promotion (plain `BAR`/`SAR` → `REAR BUY`/`SELL`) vs. every later rung-to-rung escalation. `REAR BUY RE ENTER` then reached its own `REAR BUY RE ENTER 2` the very next day, 11-03-2022 — the first real observation of a "2" stage anywhere on the ladder past the first rung.
