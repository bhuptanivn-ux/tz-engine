...
## TypeScript port (`lib/tzEngineNewTheory.ts`)

The live site (a Next.js app on Vercel, which has no Python runtime) runs a
line-for-line TypeScript port of `tz_engine_new_theory.py`, feeding it real
OHLC fetched from Yahoo Finance for whatever stock/index and date range the
user selects, and rendering the result as an "Event" column. The port was
verified byte-identical to the Python original by running both against
`test_new_theory_smoke.py`'s two synthetic sequences and diffing the
per-day event output — every event, on every day, in the same order,
matched exactly.

Porting does **not** change the theory's verification status above: this
is still the same provisional, unverified logic, now just runnable against
real data instead of requiring a manually-prepared CSV/XLSX file. One
inherited detail worth flagging for real data specifically: every threshold
in this theory (`THRESH = 0.20`, `ANY = 0.01`) is an **absolute** price
unit, not a percentage. It was seemingly tuned against one specific price
range; it has not been re-examined for stocks/indices priced very
differently (e.g. a S&P 500 or Nasdaq 100 index trading in the thousands,
where 0.20 is noise, versus a low-priced stock where it may be too coarse).
Treat Event output on anything outside the original tuning range with
extra suspicion until checked.

One piece of the Python source was deliberately not ported:
`_eval_rear_reenter_tracking` is dead code in the original (never called,
and references Cycle fields that don't exist — see the port file's header
comment for detail).
